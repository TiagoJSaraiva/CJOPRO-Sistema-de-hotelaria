...
| 6 — Expansão do PMS      | Em implementação     | Reservas e tarifas validadas; relacionamento e pré-chegada validados; API pública, canais e indicadores em validação | Categoria sem quarto prometido; snapshots; token por hash | 483 verificações SQL; testes direcionados de shared, backend, PMS e aplicação pública aprovados | Validar blocos 3 e 4 e executar a limpeza delimitada |
