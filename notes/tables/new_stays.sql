-- Modelo consolidado de reservas e estadias
-- Regras:
-- 1) Reserva agrupa estadias
-- 2) Estadia representa ocupacao por quarto e periodo
-- 3) Hospedes da estadia entram no check-in (stay_customers)
...
create table public.reservations (
  ...
  constraint reservations_hotel_id_fkey foreign key (hotel_id) references hotels (id) on delete restrict,
  constraint reservations_booking_customer_id_fkey foreign key (booking_customer_id) references customers (id) on delete restrict
);
...
create table public.stays (
  ...
  constraint stays_reservation_id_fkey foreign key (reservation_id) references reservations (id) on delete cascade,
...
create table public.stay_consumption (
  ...
  item_total_amount numeric generated always as (((quantity)::numeric * charged_unit_price)) stored,
  ...
  constraint stay_consumption_product_id_fkey foreign key (product_id) references products (id) on delete restrict,
  constraint stay_consumption_stay_id_fkey foreign key (stay_id) references stays (id) on delete cascade
);
...
create table public.stay_customers (
  ...
  constraint stay_customers_stay_id_customer_id_key unique (stay_id, customer_id),
  constraint stay_customers_customer_id_fkey foreign key (customer_id) references customers (id) on delete restrict,
  constraint stay_customers_stay_id_fkey foreign key (stay_id) references stays (id) on update cascade on delete cascade
);
