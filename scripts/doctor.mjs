...
const ESSENTIAL_CLIS = [
  ...
  { name: "TypeScript", directory: ".", packageName: "typescript", binName: "tsc" },
  { name: "ESLint", directory: ".", packageName: "eslint", binName: "eslint" },
  { name: "Vitest", directory: ".", packageName: "vitest", binName: "vitest" },
  { name: "Supabase CLI", directory: ".", packageName: "supabase", binName: "supabase" },
  { name: "Next.js", directory: "apps/pms", packageName: "next", binName: "next" },
  { name: "Playwright", directory: "apps/pms", packageName: "@playwright/test", binName: "playwright" },
  { name: "tsx (backend)", directory: "apps/backend-service", packageName: "tsx", binName: "tsx" },
  { name: "tsup (backend)", directory: "apps/backend-service", packageName: "tsup", binName: "tsup" },
  { name: "tsx (booking)", directory: "apps/booking-engine-service", packageName: "tsx", binName: "tsx" },
  { name: "tsup (booking)", directory: "apps/booking-engine-service", packageName: "tsup", binName: "tsup" },
  { name: "tsup (shared)", directory: "packages/shared", packageName: "tsup", binName: "tsup" }
];
...
function readJson(relativePath) {
  return JSON.parse(readFileSync(resolve(ROOT_DIRECTORY, relativePath), "utf8"));
}

function getLockfileImporters(lockfileContents) {
  const importersSection = lockfileContents.split(/^packages:\s*$/m)[0]?.split(/^importers:\s*$/m)[1] || "";
...
function resolveCliBinary(cli) {
  ...
  const packageJsonPath = resolve(ROOT_DIRECTORY, cli.directory, "node_modules", ...packageParts, "package.json");
...
addCheck("node-version", "Versão do Node.js", process.versions.node === EXPECTED_NODE_VERSION, {
...
const pnpmVersion = getCommandOutput(pnpmVersionResult).split(/\r?\n/).at(-1) || "indisponível";
addCheck("pnpm-version", "Versão do pnpm", pnpmVersionResult.status === 0 && pnpmVersion === EXPECTED_PNPM_VERSION, {
...
const nvmVersion = readFileSync(resolve(ROOT_DIRECTORY, ".nvmrc"), "utf8").trim();
addCheck("nvm-version", "Pinagem do .nvmrc", nvmVersion === EXPECTED_NODE_VERSION, {
  expected: EXPECTED_NODE_VERSION,
  actual: nvmVersion,
...
const missingWorkspaces = EXPECTED_WORKSPACES.filter(
  (workspace) => workspace !== "." && !existsSync(resolve(ROOT_DIRECTORY, workspace, "package.json"))
);
addCheck("workspace-layout", "Diretórios do workspace", missingWorkspaces.length === 0, {
  expected: EXPECTED_WORKSPACES.join(", "),
  actual: missingWorkspaces.length === 0 ? "todos presentes" : `ausentes: ${missingWorkspaces.join(", ")}`,
...
if (existsSync(lockfilePath)) {
  const lockfileImporters = getLockfileImporters(readFileSync(lockfilePath, "utf8"));
  const missingImporters = EXPECTED_WORKSPACES.filter((workspace) => !lockfileImporters.includes(workspace));
  const unexpectedImporters = lockfileImporters.filter((workspace) => !EXPECTED_WORKSPACES.includes(workspace));
  const lockfileOk = missingImporters.length === 0 && unexpectedImporters.length === 0;
  const details = [];
  if (missingImporters.length) details.push(`ausentes: ${missingImporters.join(", ")}`);
  if (unexpectedImporters.length) details.push(`obsoletos: ${unexpectedImporters.join(", ")}`);
...
const modulesManifest = resolve(ROOT_DIRECTORY, "node_modules", ".modules.yaml");
const dependenciesInstalled = existsSync(modulesManifest);
addCheck("dependencies-installed", "Instalação das dependências", dependenciesInstalled, {
  expected: "node_modules consistente",
  actual: dependenciesInstalled ? "manifesto do pnpm presente" : "node_modules ausente ou incompleto",
...
if (dependenciesInstalled) {
  ...
  addCheck("essential-clis", "CLIs essenciais", unavailableClis.length === 0, {
    ...
    actual: unavailableClis.length === 0 ? "todos resolvidos" : `indisponíveis: ${unavailableClis.join(", ")}`,
...
if (JSON_OUTPUT) {
  ...
  for (const check of checks) {
    process.stdout.write(`${check.ok ? "[OK]" : "[FALHA]"} ${check.label}: ${check.actual || "sem detalhes"}\n`);
    if (!check.ok) {
