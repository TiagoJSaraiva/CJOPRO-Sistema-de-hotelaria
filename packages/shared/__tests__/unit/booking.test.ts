...
describe("booking rules", () => {
  it("calcula garantias e penalidades sem ultrapassar regras monetárias", () => {
    ...
    expect(calculateGuarantee({ type: "fixed", value: 120 }, 900, 300)).toBe(
      120,
    );
    expect(
      calculateGuarantee({ type: "percentage", value: 30 }, 900, 300),
    ).toBe(270);
    expect(
      calculateGuarantee({ type: "first_night", value: 0 }, 900, 300),
    ).toBe(300);
    expect(
      calculateCancellationPenalty({ type: "none", value: 0 }, 900, 300),
    ).toBe(0);
    expect(
      calculateCancellationPenalty({ type: "fixed", value: 100 }, 900, 300),
    ).toBe(100);
    expect(
      calculateCancellationPenalty({ type: "percentage", value: 10 }, 900, 300),
    ).toBe(90);
    expect(
      calculateCancellationPenalty({ type: "first_night", value: 0 }, 900, 300),
    ).toBe(300);
    expect(
      calculateCancellationPenalty({ type: "full_stay", value: 0 }, 900, 300),
    ).toBe(900);
  });
