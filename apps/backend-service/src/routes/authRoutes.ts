...
function getScopedAuthUser(
  payload: SessionPayload,
  activeHotelId: string | null,
): MeSuccessResponse["user"] {
  const assignments = payload.roleAssignments || [];
  const hasPerAssignmentPermissions = assignments.some((assignment) =>
...
export function registerAuthRoutes(
  ...
  app.post<{ Body: LoginBody }>("/auth/login", async (request, reply) => {
    ...
    if (userRow === undefined) {
      return reply
        .status(500)
        .send({ message: "Falha ao autenticar usuario." });
    }
    ...
    if (lockedUntil && !Number.isNaN(lockedUntil.getTime())) {
      ...
      const clearLockResult = await repository
        .clearExpiredLoginLock(userRow.id)
        .catch((error) => {
          request.log.error(error);
          return false;
        });
    ...
    const passwordMatches = await matchesPasswordHash(
      password,
    ...
    if (!passwordMatches) {
      ...
      if (shouldLock && lockUntil) {
        const retryAfterSeconds = Math.max(
          1,
          Math.ceil((lockUntil.getTime() - now.getTime()) / 1000),
        );
        reply.header("Retry-After", String(retryAfterSeconds));
        return reply.status(429).send(accountLockedError(retryAfterSeconds));
      }
  ...
  app.get("/auth/me", async (request, reply) => {
    ...
    if (!contextResult.ok) {
      return reply
        ...
        .send({ message: contextResult.message });
    }
