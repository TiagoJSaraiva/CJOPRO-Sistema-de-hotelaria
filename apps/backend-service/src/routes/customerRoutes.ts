...
export function registerCustomerRoutes(
  app: FastifyInstance,
  repository: CustomersRepository = createCustomersRepository(),
): void {
  app.get("/admin/customers", async (request, reply) => {
    ...
    if (!data)
      return reply
        .status(500)
        .send(
          adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao consultar clientes."),
        );

    return reply.send({ items: data });
  });

  app.post<{ Body: CustomerCreateBody }>(
    "/admin/customers",
    async (request, reply) => {
      const auth = ensureAuthorizedWithScope(
        request,
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Nome, documento, tipo de documento e data de nascimento sao obrigatorios.",
            ),
          );
      }
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Falha ao criar cliente.",
              getAdminErrorDetails(createError),
            ),
          );
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Cliente ja cadastrado com esse documento.",
            ),
          );
      if (!createResult.item)
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao criar cliente."),
          );
  ...
  app.put<{ Params: HotelIdParams; Body: CustomerUpdateBody }>(
    "/admin/customers/:id",
    async (request, reply) => {
      const auth = ensureAuthorizedWithScope(
        request,
      ...
      if (!id)
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id do cliente e obrigatorio para atualizacao.",
            ),
          );

      const payload: TablesUpdate<"customers"> = {};
      if (request.body?.full_name !== undefined)
        payload.full_name = normalizeOptionalText(request.body.full_name) || "";
      if (request.body?.document_number !== undefined)
        payload.document_number =
          normalizeOptionalText(request.body.document_number) || "";
      if (request.body?.document_type !== undefined)
        payload.document_type =
          normalizeOptionalText(request.body.document_type) || "";
      if (request.body?.email !== undefined)
        payload.email = normalizeOptionalText(request.body.email);
      if (request.body?.mobile_phone !== undefined)
        payload.mobile_phone = normalizeOptionalText(request.body.mobile_phone);
      if (request.body?.phone !== undefined)
        payload.phone = normalizeOptionalText(request.body.phone);
      if (request.body?.birth_date !== undefined)
        payload.birth_date =
          normalizeOptionalText(request.body.birth_date) || "";
      if (request.body?.nationality !== undefined)
        payload.nationality = normalizeOptionalText(request.body.nationality);
      if (request.body?.notes !== undefined)
        payload.notes = normalizeOptionalText(request.body.notes);
      ...
      const updateResult = await repository
        .updateCustomer(id, activeHotelId, payload)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Cliente nao encontrado neste hotel.",
            ),
          );
      if (updateResult.result === "conflict")
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Conflito de dados ao atualizar cliente.",
            ),
          );
    ...
    async (request, reply) => {
      ...
      if (!id)
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id do cliente e obrigatorio para exclusao.",
            ),
          );

      const result = await repository
        .deleteCustomer(id, activeHotelId)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
      ...
      if (!result)
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao excluir cliente."),
          );
      if (result === "not-found")
        return reply
          .status(404)
      ...
      if (result === "conflict")
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Cliente nao pode ser excluido: possui dependencias ativas.",
            ),
          );
