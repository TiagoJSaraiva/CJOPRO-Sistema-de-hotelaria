...
function parsePermissionType(value: unknown): AdminPermissionType | null {
  if (
    value === ADMIN_PERMISSION_TYPES.SYSTEM ||
    value === ADMIN_PERMISSION_TYPES.HOTEL
  ) {
    return value;
  }
...
export function registerPermissionRoutes(
      ...
      return reply
        ...
        .send(
          adminError(
            ...
            "Falha ao consultar permissoes.",
          ),
        );
    }
  });

  app.post<{ Body: PermissionCreateBody }>(
    "/admin/permissions",
    async (request, reply) => {
      if (
        !ensureAuthorizedSystem(request, reply, PERMISSIONS.PERMISSION_CREATE)
      ) {
        return;
      }
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Nome da permissao e obrigatorio.",
            ),
          );
      }
      ...
      if (!type) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Tipo da permissao e obrigatorio.",
            ),
          );
      }
      ...
      if (!createResult) {
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao criar permissao."),
          );
      }
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Nome de permissao ja existente.",
            ),
          );
      }
  ...
  app.put<{ Params: HotelIdParams; Body: PermissionUpdateBody }>(
    ...
    async (request, reply) => {
      ...
      const type =
        request.body?.type !== undefined
          ? parsePermissionType(request.body?.type)
          : undefined;
      ...
      if (!id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id da permissao e obrigatorio para atualizacao.",
            ),
          );
      }
      ...
      if (permission === undefined) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.INTERNAL,
              "Falha ao consultar permissao para atualizacao.",
            ),
          );
      }
      ...
      if (!permission) {
        return reply
          .status(404)
          .send(
            adminError(ADMIN_ERROR_CODE.NOT_FOUND, "Permissao nao encontrada."),
          );
      }

      const hasSelfPermissionBySession = session.permissions.includes(
        permission.name,
      );
      const hasSelfPermissionByAssignment = (
        session.roleAssignments || []
      ).some((assignment) =>
        (assignment.permissions || []).includes(permission.name),
      );
      ...
      if (request.body?.name !== undefined && !name) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Nome da permissao e obrigatorio para atualizacao.",
            ),
          );
      }
      ...
      if (request.body?.type !== undefined && !type) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Tipo da permissao invalido para atualizacao.",
            ),
          );
      }
      ...
      if (!id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id da permissao e obrigatorio para exclusao.",
            ),
          );
      }
      ...
      if (permission === undefined) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.INTERNAL,
              "Falha ao consultar permissao para exclusao.",
            ),
          );
      }
      ...
      if (deleteResult === "conflict") {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Permissao nao pode ser excluida: possui dependencias ativas.",
            ),
          );
      }
