...
import {
  ensureAuthorizedSystem,
  ensureAuthorizedAnySystem,
...
export function registerRoleRoutes(
  app: FastifyInstance,
  repository: RolesRepository = createRolesRepository(),
): void {
  app.get("/admin/roles/reference-data", async (request, reply) => {
    if (
      !ensureAuthorizedAnySystem(request, reply, [
    ...
    try {
      const [hotels, permissions] = await Promise.all([
        ...
        repository.listReferencePermissions(),
      ]);
      ...
      return reply
        ...
        .send(
          adminError(
            ...
            "Falha ao consultar dados auxiliares de roles.",
          ),
        );
    }
  });
  ...
  app.get("/admin/roles", async (request, reply) => {
    ...
    try {
      ...
      return reply
        .status(500)
        .send(
          adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao consultar roles."),
        );
    }
  });
  ...
  app.post<{ Body: RoleCreateBody }>("/admin/roles", async (request, reply) => {
    ...
    if (roleType === ADMIN_ROLE_TYPES.SYSTEM && hotelId) {
      return reply
        ...
        .send(
          adminError(
            ADMIN_ERROR_CODE.VALIDATION,
            "Roles de sistema nao podem ter hotel associado.",
          ),
        );
    }
      ...
      if (exists === null) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.INTERNAL,
              "Falha ao validar hotel da role.",
            ),
          );
      }
    ...
    if (permissionIds.length) {
      const permissions = await repository
        .findPermissionsByIds(permissionIds)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
      ...
      if (permissions === null) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.INTERNAL,
              "Falha ao validar permissoes da role.",
            ),
          );
      }
      ...
      if (permissions.length !== permissionIds.length) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Uma ou mais permissoes selecionadas nao existem.",
            ),
          );
      }
      ...
      const hasInvalidPermissionType = permissions.some(
        (item) => item.type !== expectedPermissionType,
      );
    ...
    const createRoleResult = await repository
      .createRoleWithPermissions(
        { name, role_type: roleType, hotel_id: hotelId },
        permissionIds,
      )
      .catch((error) => {
        request.log.error(error);
    ...
    if (!createRoleResult) {
      return reply
        .status(500)
        .send(adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao criar role."));
    }
    ...
    if (createRoleResult.result === "conflict") {
      return reply
        .status(409)
        .send(
          adminError(ADMIN_ERROR_CODE.CONFLICT, "Nome de role ja existente."),
        );
    }
    ...
    const roleWithRelations = await repository
      .getRoleWithRelationsById(createRoleResult.id)
      .catch((error) => {
        request.log.error(error);
        return null;
      });
    ...
    if (!roleWithRelations) {
      return reply
        ...
        .send(
          adminError(
            ADMIN_ERROR_CODE.INTERNAL,
            "Falha ao consultar role criada.",
          ),
        );
    }
  ...
  app.put<{ Params: HotelIdParams; Body: RoleUpdateBody }>(
    "/admin/roles/:id",
    async (request, reply) => {
      const session = ensureAuthorizedSystem(
        request,
        reply,
        PERMISSIONS.ROLE_UPDATE,
      );
      ...
      if (!id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id da role e obrigatorio para atualizacao.",
            ),
          );
      }

      const isSelfRole = (session.roleAssignments || []).some(
        ...
        if (!parsedName) {
          return reply
            ...
            .send(
              adminError(
                ADMIN_ERROR_CODE.VALIDATION,
                "Nome da role nao pode ficar vazio.",
              ),
            );
        }
        ...
        if (!parsedRoleType) {
          return reply
            ...
            .send(
              adminError(
                ADMIN_ERROR_CODE.VALIDATION,
                "Tipo da role invalido para atualizacao.",
              ),
            );
        }
      ...
      const permissionIds = hasPermissionsPayload
        ? normalizePermissionIds(request.body?.permission_ids)
        : [];
      ...
      const currentRole = await repository
        .getRoleWithRelationsById(id)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
      ...
      if (!currentRole) {
        return reply
          .status(404)
          .send(adminError(ADMIN_ERROR_CODE.NOT_FOUND, "Role nao encontrada."));
      }

      const effectiveRoleType =
        parseRoleType(payload.role_type) ||
      ...
      const updateResult = await repository
        .updateRoleWithPermissions(
          id,
          payload,
          hasPermissionsPayload ? permissionIds : undefined,
        )
        .catch((error) => {
          request.log.error(error);
          return null;
      ...
      if (!updateResult) {
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao atualizar role."),
          );
      }
      ...
      if (!updatedRole) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.INTERNAL,
              "Falha ao consultar role atualizada.",
            ),
          );
      }
    ...
    async (request, reply) => {
      ...
      if (!id) {
        ...
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id da role e obrigatorio para exclusao.",
            ),
          );
      }
      ...
      if (isSelfRole) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.SELF_ACTION_FORBIDDEN,
              "Nao e permitido excluir uma role vinculada ao proprio usuario.",
            ),
          );
      }
      ...
      if (!deleteResult) {
        ...
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao excluir role."),
          );
      }
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Role nao pode ser excluida: possui dependencias ativas.",
            ),
          );
      }
