...
import {
  createReservationsCalendarRepository,
  type ReservationsCalendarRepository,
} from "../repositories/reservationsCalendarRepository";
...
function buildContinuousGroups(
  selectedCells: Array<{ room_id: string; date: string }>,
): GroupedSelection[] {
  ...
  for (const [roomId, datesRaw] of groupedByRoom.entries()) {
    ...
    for (let index = 1; index < dates.length; index++) {
      ...
      const prevPlusOne = dateToIso(
        new Date(parseIsoDate(prev).getTime() + 86400000),
      );
      if (current !== prevPlusOne) {
        const nights =
          Math.round(
            (parseIsoDate(prev).getTime() - parseIsoDate(start).getTime()) /
              86400000,
          ) + 1;
        result.push({
          room_id: roomId,
...
async function computeBooking(
  ...
  | {
      ok: true;
      data: AdminReservationCalendarBookingCreateResponse;
      groups: GroupedSelection[];
      roomsById: Map<string, RoomRow>;
    }
  | { ok: false; statusCode: number; message: string }
> {
  const selectedCells = Array.isArray(payload.selected_cells)
  ...
  if (!selectedCells.length) {
    return {
      ok: false,
      statusCode: 400,
      message: "Selecione ao menos uma celula livre.",
    };
  }
  ...
  const roomIds = Array.from(
    new Set(normalizedCells.map((cell) => cell.room_id)),
  );
  const minDate = normalizedCells.map((cell) => cell.date).sort()[0]!;
  const maxDate = normalizedCells.map((cell) => cell.date).sort()[
  ...
  if (roomsResult.error) {
    return {
      ok: false,
      statusCode: 500,
      message: "Falha ao consultar quartos selecionados.",
    };
  }
  ...
  if (rooms.length !== roomIds.length) {
    return {
      ok: false,
      statusCode: 400,
      message: "Um ou mais quartos selecionados nao pertencem ao hotel ativo.",
    };
  }
  ...
  if (
    ...
    return {
      ok: false,
      statusCode: 409,
      message: "Um ou mais quartos estão indisponíveis para novas reservas.",
    };
  }
  ...
  if (conflictResult.error) {
    return {
      ok: false,
      statusCode: 500,
      message: "Falha ao validar conflitos de disponibilidade.",
    };
  }
  ...
  for (const cell of normalizedCells) {
    ...
    if (hasConflict) {
      return {
        ...
        statusCode: 409,
        message:
          "Conflito de disponibilidade em uma ou mais celulas selecionadas.",
      };
    }
  }
  ...
  if (blocksResult.error) {
    return {
      ok: false,
      statusCode: 500,
      message: "Falha ao validar bloqueios de manutenção.",
    };
  }
  const blockedSelection = normalizedCells.some((cell) =>
    (
      ...
      const effectiveEnd =
        block.end_date < today ? "9999-12-31" : block.end_date;
      return (
        block.room_id === cell.room_id &&
        cell.date >= block.start_date &&
        cell.date <= effectiveEnd
      );
    }),
  );
  if (blockedSelection) {
    return {
      ok: false,
      statusCode: 409,
      message: "Um ou mais quartos estão bloqueados por manutenção.",
    };
  }
  ...
  if (seasonsResult.error) {
    return {
      ok: false,
      statusCode: 500,
      message: "Falha ao consultar temporadas.",
    };
  }
  ...
  if (seasonIds.length) {
    ...
    if (ratesResult.error) {
      return {
        ok: false,
        statusCode: 500,
        message: "Falha ao consultar tarifas de temporada.",
      };
    }
    ratesByKey = new Map(
      ((ratesResult.data || []) as SeasonRateRow[]).map((row) => [
        `${row.season_id}::${row.room_type}`,
  ...
  let totalPrice = 0;
  for (const cell of normalizedCells.filter(
    (item) => item.side !== "checkout",
  )) {
    const room = roomsById.get(cell.room_id)!;
    const season = getOverlappingSeason(cell.date, seasons);
    const extra = season
      ? ratesByKey.get(`${season.id}::${room.room_type}`) || 0
      : 0;
  ...
  const groups = buildContinuousGroups(
    normalizedCells.filter((item) => item.side !== "checkout"),
  );
  if (!groups.length) {
    return {
      ok: false,
...
export function registerReservationsCalendarRoutes(
  ...
  app.get<{ Querystring: CalendarQuery }>(
    ...
    async (request, reply) => {
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "start_date invalido. Use o formato YYYY-MM-DD.",
            ),
          );
      }
      ...
      if (
        ...
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              `days invalido. Informe um valor entre 1 e ${MAX_DAYS}.`,
            ),
          );
      }
      ...
      const timeline = await repository
        .getTimeline(activeHotelId, startDate, endDate)
        .catch((error) => {
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Falha ao consultar calendário de reservas.",
            ),
          );
      }
  ...
  app.post<{ Body: Partial<AdminReservationCalendarBookingCreateInput> }>(
    ...
    async (request, reply) => {
      ...
      if (!computed.ok) {
        return reply
          .status(computed.statusCode)
          .send(adminError(ADMIN_ERROR_CODE.VALIDATION, computed.message));
      }
  ...
  app.post<{ Body: Partial<AdminReservationCalendarBookingCreateInput> }>(
    ...
    async (request, reply) => {
      ...
      if (!computed.ok) {
        const code =
          computed.statusCode === 409
            ? ADMIN_ERROR_CODE.CONFLICT
            : ADMIN_ERROR_CODE.VALIDATION;
        return reply
          .status(computed.statusCode)
          .send(adminError(code, computed.message));
      }
      ...
      if (bookingCustomer.mode === "existing") {
        ...
        if (!bookingCustomerId) {
          return reply
            ...
            .send(
              adminError(
                ADMIN_ERROR_CODE.VALIDATION,
                "customer_id obrigatorio para cliente existente.",
              ),
            );
        }
      } else if (bookingCustomer.mode === "create_inline") {
        const customersRepository = createCustomersRepository();
        const createCustomerResult = await customersRepository.createCustomer(
          activeHotelId,
          {
            full_name: normalizeOptionalText(bookingCustomer.full_name) || "",
            document_number:
              normalizeOptionalText(bookingCustomer.document_number) || "",
            document_type:
              normalizeOptionalText(bookingCustomer.document_type) || "",
        ...
        if (
          createCustomerResult.result !== "ok" ||
          !createCustomerResult.item
        ) {
          return reply
            .status(409)
            .send(
        ...
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Modo de booking_customer invalido.",
            ),
          );
      }
      ...
      for (let attempts = 0; attempts < 10; attempts++) {
        ...
        const existing = await reservationsRepository
          .listReservations(activeHotelId)
          .catch(() => []);
      ...
      if (!reservationCode) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.INTERNAL,
              "Falha ao gerar codigo de reserva.",
            ),
          );
      }

      const createReservationResult =
        await reservationsRepository.createReservation(activeHotelId, {
      ...
      if (
        createReservationResult.result !== "ok" ||
        !createReservationResult.item
      ) {
        return reply
          .status(409)
          .send(
            adminError(ADMIN_ERROR_CODE.CONFLICT, "Falha ao criar reserva."),
          );
      }
      ...
      for (const group of computed.groups) {
        ...
        const groupTotal = computed.data.breakdown
          .filter(
            (item) =>
              item.room_id === group.room_id &&
              item.date >= group.start_date &&
              item.date <= group.end_date,
          )
        ...
        if (stayError || !stayData?.id) {
          ...
          const details =
            stayError && typeof stayError === "object"
              ? [
                  String((stayError as { code?: string }).code || ""),
                  String((stayError as { message?: string }).message || ""),
                ]
                  .filter(Boolean)
                  .join(" | ") || undefined
              : undefined;
          return reply
            ...
            .send(
              adminError(
                ADMIN_ERROR_CODE.CONFLICT,
                "Falha ao criar estadias da reserva.",
                details,
              ),
            );
        }
