...
function normalizeOptionalDateOnly(value: string | null | undefined): string | null | undefined {
  const text = normalizeOptionalText(value);
  if (!text) return null;
...
function normalizeOptionalDateTime(value: string | null | undefined): string | null | undefined {
  const text = normalizeOptionalText(value);
  if (!text) return null;
...
export function registerFinancialTransactionRoutes(
  ...
  app.post<{ Body: FinancialTransactionCreateBody }>("/admin/financial-transactions", async (request, reply) => {
    ...
    if (!isTransactionType(type) || !category || !Number.isFinite(amount) || amount < 0 || !currency) {
    ...
    if (paidAt === undefined || dueDate === undefined || stayId === undefined || reservationId === undefined) {
      return reply.status(400).send(adminError(ADMIN_ERROR_CODE.VALIDATION, "Datas ou vinculos da transacao invalidos."));
    }
    ...
    const createResult = await repository
      .createFinancialTransaction(activeHotelId, {
        ...
        paid_at: shouldStampPaidAt(resolvedStatus) ? paidAt || new Date().toISOString() : null,
        due_date: dueDate,
        counterparty,
  ...
  app.put<{ Params: HotelIdParams; Body: FinancialTransactionUpdateBody }>("/admin/financial-transactions/:id", async (request, reply) => {
    ...
    if (request.body?.payment_method !== undefined) {
      payload.payment_method = normalizeOptionalText(request.body.payment_method);
    }

    if (request.body?.paid_at !== undefined) {
      const paidAt = normalizeOptionalDateTime(request.body.paid_at);
      if (paidAt === undefined) {
        return reply.status(400).send(adminError(ADMIN_ERROR_CODE.VALIDATION, "Data de pagamento invalida."));
      }
    ...
    if (request.body?.due_date !== undefined) {
      ...
      if (dueDate === undefined) {
        return reply.status(400).send(adminError(ADMIN_ERROR_CODE.VALIDATION, "Data de vencimento invalida."));
      }
    ...
    if (request.body?.reference_code !== undefined) {
      payload.reference_code = normalizeOptionalText(request.body.reference_code);
    }

    if (request.body?.stay_id !== undefined) {
      const stayId = normalizeOptionalUuid(request.body.stay_id);
      if (stayId === undefined) {
        return reply.status(400).send(adminError(ADMIN_ERROR_CODE.VALIDATION, "Id da estadia invalido."));
      }
    ...
    if (request.body?.reservation_id !== undefined) {
      ...
      if (reservationId === undefined) {
        return reply.status(400).send(adminError(ADMIN_ERROR_CODE.VALIDATION, "Id da reserva invalido."));
      }
    ...
    if (payload.status) {
      ...
      payload.paid_at = shouldStampPaidAt(nextStatus) ? payload.paid_at || new Date().toISOString() : null;
    }
