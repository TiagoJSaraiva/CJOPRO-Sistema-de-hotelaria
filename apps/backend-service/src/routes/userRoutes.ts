...
> {
  ...
  const uniqueRoleIds = Array.from(
    new Set(roleAssignments.map((item) => item.role_id)),
  );
  ...
  try {
    ...
    return {
      ok: false,
      status: 500,
      message: "Falha ao validar papeis para o usuario.",
    };
  }
  ...
  if (roleMap.size !== uniqueRoleIds.length) {
    return {
      ok: false,
      status: 400,
      message: "Uma ou mais roles selecionadas nao existem.",
    };
  }

  const selectedHotelIds = Array.from(
    new Set(
      roleAssignments
  ...
  if (selectedHotelIds.length) {
    try {
      ...
      return {
        ok: false,
        status: 500,
        message: "Falha ao validar hoteis para o usuario.",
      };
    }
  }
  ...
  for (const assignment of roleAssignments) {
    ...
    if (!role) {
      return {
        ok: false,
        status: 400,
        message: "Role selecionada nao existe.",
      };
    }
    ...
    if (role.role_type === ADMIN_ROLE_TYPES.SYSTEM) {
      if (assignment.hotel_id) {
        return {
          ok: false,
          status: 400,
          message: "Roles de sistema devem ser vinculadas no contexto Sistema.",
        };
      }
    ...
    if (role.hotel_id) {
      if (assignment.hotel_id && assignment.hotel_id !== role.hotel_id) {
        return {
          ok: false,
          status: 400,
          message: "A role selecionada nao pertence ao hotel escolhido.",
        };
      }
    ...
    if (!validHotelIds.has(assignment.hotel_id)) {
      return {
        ok: false,
        status: 400,
        message: "Hotel selecionado nao existe.",
      };
    }
...
export function registerUserRoutes(
  app: FastifyInstance,
  repository: UsersRepository = createUsersRepository(),
): void {
  app.get("/admin/users/reference-data", async (request, reply) => {
    if (
      !ensureAuthorizedAny(request, reply, [
    ...
    try {
      const [hotels, roles] = await Promise.all([
        repository.listReferenceHotels(),
        repository.listReferenceRoles(),
      ]);
      ...
      return reply
        ...
        .send(
          adminError(
            ...
            "Falha ao consultar dados auxiliares de usuarios.",
          ),
        );
    }
  });
  ...
  app.get("/admin/users", async (request, reply) => {
    const authWithScope = ensureAuthorizedWithScope(
      request,
      reply,
      PERMISSIONS.USER_READ,
    );
    ...
    try {
      ...
      return reply
        .status(500)
        .send(
          adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao consultar usuarios."),
        );
    }
  });
  ...
  app.post<{ Body: UserCreateBody }>("/admin/users", async (request, reply) => {
      ...
      return reply
        ...
        .send(
          adminError(
            ...
            "Nome, email e senha temporaria sao obrigatorios.",
          ),
        );
    }
    ...
    const roleAssignmentsValidation = await validateAndNormalizeRoleAssignments(
      repository,
      roleAssignments,
    );
    ...
    if (!roleAssignmentsValidation.ok) {
      return reply
        .status(roleAssignmentsValidation.status)
        .send(
          adminError(
            ADMIN_ERROR_CODE.VALIDATION,
            roleAssignmentsValidation.message,
          ),
        );
    }

    const passwordHash = await hashTemporaryPassword(tempPassword).catch(
      (error) => {
        request.log.error(error);
        return null;
      },
    ...
    if (!passwordHash) {
      return reply
        .status(500)
        .send(adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao criar usuario."));
    }
    ...
    const userWithRelations = await repository
      .getUserWithRelationsById(createResult.id, authWithScope.activeHotelId)
      .catch((error) => {
        request.log.error(error);
        return null;
      });
    ...
    if (!userWithRelations) {
      return reply
        ...
        .send(
          adminError(
            ADMIN_ERROR_CODE.INTERNAL,
            "Falha ao consultar usuario criado.",
          ),
        );
    }
  ...
  app.put<{ Params: HotelIdParams; Body: UserUpdateBody }>(
    "/admin/users/:id",
    async (request, reply) => {
      const authWithScope = ensureAuthorizedWithScope(
        request,
      ...
      if (!id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id do usuario e obrigatorio para atualizacao.",
            ),
          );
      }
      ...
      if (id === authWithScope.session.id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.SELF_ACTION_FORBIDDEN,
              "Nao e permitido atualizar o proprio usuario por esta rota.",
            ),
          );
      }

      const userToUpdate = await repository
        .getUserWithRelationsById(id, authWithScope.activeHotelId)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Usuario nao encontrado ou nao tem acesso neste contexto.",
            ),
          );
      }
      ...
      if (request.body?.password_hash !== undefined) {
        const parsedPassword = normalizeOptionalText(
        ...
        if (!parsedPassword) {
          return reply
            ...
            .send(
              adminError(
                ADMIN_ERROR_CODE.VALIDATION,
                "Senha temporaria nao pode ficar vazia.",
              ),
            );
        }

        const passwordHash = await hashTemporaryPassword(parsedPassword).catch(
          (error) => {
            request.log.error(error);
            return null;
          },
      ...
      const roleAssignments = hasRoleAssignments
        ? normalizeRoleAssignments(request.body?.role_assignments)
        : [];
      ...
      let normalizedAssignments:
        Array<{ role_id: string; hotel_id: string | null }> | undefined;

      if (hasRoleAssignments) {
        const roleAssignmentsValidation =
          await validateAndNormalizeRoleAssignments(
            repository,
            roleAssignments,
          );
      ...
      const updateResult = await repository
        .updateUserWithRoles(
          id,
          payload,
          hasRoleAssignments ? normalizedAssignments || [] : undefined,
        )
        .catch((error) => {
          request.log.error(error);
          return null;
      ...
      if (updateResult === "not-found") {
        return reply
          .status(404)
          .send(
            adminError(ADMIN_ERROR_CODE.NOT_FOUND, "Usuario nao encontrado."),
          );
      }
      ...
      if (!updatedUser) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.INTERNAL,
              "Falha ao consultar usuario atualizado.",
            ),
          );
      }
    ...
    async (request, reply) => {
      ...
      if (!id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id do usuario e obrigatorio para exclusao.",
            ),
          );
      }
      ...
      if (id === authWithScope.session.id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.SELF_ACTION_FORBIDDEN,
              "Nao e permitido excluir o proprio usuario por esta rota.",
            ),
          );
      }

      const userToDelete = await repository
        .getUserWithRelationsById(id, authWithScope.activeHotelId)
        .catch((error) => {
          request.log.error(error);
          return null;
      ...
      if (!deleteResult) {
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao excluir usuario."),
          );
      }
      ...
      if (deleteResult === "conflict") {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Usuario nao pode ser excluido: possui dependencias ativas.",
            ),
          );
      }
