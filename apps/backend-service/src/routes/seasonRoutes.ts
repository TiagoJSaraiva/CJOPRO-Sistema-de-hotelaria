...
export function registerSeasonRoutes(
  app: FastifyInstance,
  repository: SeasonsRepository = createSeasonsRepository(),
): void {
  ...
  app.post<{ Body: SeasonCreateBody }>(
    "/admin/seasons",
    async (request, reply) => {
      const auth = ensureAuthorizedWithScope(
        request,
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Nome, data de inicio e data de fim sao obrigatorios.",
            ),
          );
      }
      ...
      if (!createResult)
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao criar temporada."),
          );
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Conflito ao criar temporada (sobreposicao ou duplicidade).",
            ),
          );
  ...
  app.put<{ Params: HotelIdParams; Body: SeasonUpdateBody }>(
    "/admin/seasons/:id",
    async (request, reply) => {
      const auth = ensureAuthorizedWithScope(
        request,
      ...
      if (!id)
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id da temporada e obrigatorio para atualizacao.",
            ),
          );
      ...
      if (request.body?.start_date !== undefined)
        payload.start_date =
          normalizeOptionalText(request.body.start_date) || "";
      if (request.body?.end_date !== undefined)
        payload.end_date = normalizeOptionalText(request.body.end_date) || "";
      ...
      const updateResult = await repository
        .updateSeason(id, activeHotelId, payload)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
      ...
      if (updateResult.result === "conflict")
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Conflito de dados ao atualizar temporada.",
            ),
          );
      ...
      if (!id)
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id da temporada e obrigatorio para exclusao.",
            ),
          );

      const result = await repository
        .deleteSeason(id, activeHotelId)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
      ...
      if (result === "conflict")
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Temporada nao pode ser excluida: possui dependencias ativas.",
            ),
          );
