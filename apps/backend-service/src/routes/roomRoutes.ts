...
export function registerRoomRoutes(
  app: FastifyInstance,
  repository: RoomsRepository = createRoomsRepository(),
): void {
  app.get("/admin/rooms", async (request, reply) => {
    ...
    if (!data) {
      return reply
        .status(500)
        .send(
          adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao consultar quartos."),
        );
    }
  ...
  app.post<{ Body: RoomCreateBody }>("/admin/rooms", async (request, reply) => {
      ...
      return reply
        ...
        .send(
          adminError(
            ...
            "Dados invalidos para criar quarto.",
          ),
        );
    }
    ...
    if (!createResult) {
      return reply
        .status(500)
        .send(adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao criar quarto."));
    }
      ...
      return reply
        ...
        .send(
          adminError(
            ...
            "Numero de quarto ja cadastrado neste hotel.",
          ),
        );
    }
  ...
  app.put<{ Params: HotelIdParams; Body: RoomUpdateBody }>(
    "/admin/rooms/:id",
    async (request, reply) => {
      const auth = ensureAuthorizedWithScope(
        request,
      ...
      if (!id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id do quarto e obrigatorio para atualizacao.",
            ),
          );
      }
      ...
      if (request.body?.room_number !== undefined)
        payload.room_number =
          normalizeOptionalText(request.body.room_number) || "";
      ...
      const updateResult = await repository
        .updateRoom(id, activeHotelId, payload)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
      ...
      if (!updateResult) {
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao atualizar quarto."),
          );
      }
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Quarto nao encontrado neste hotel.",
            ),
          );
      }
      ...
      if (updateResult.result === "conflict") {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Conflito de dados ao atualizar quarto.",
            ),
          );
      }
    ...
    async (request, reply) => {
      ...
      if (!id) {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id do quarto e obrigatorio para exclusao.",
            ),
          );
      }

      const result = await repository
        .deleteRoom(id, activeHotelId)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
      ...
      if (!result) {
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao excluir quarto."),
          );
      }
      ...
      if (result === "conflict") {
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Quarto nao pode ser excluido: possui dependencias ativas.",
            ),
          );
      }
