...
export function registerProductRoutes(
  ...
  app.get("/admin/products", async (request, reply) => {
    ...
    if (!data)
      return reply
        .status(500)
        .send(
          adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao consultar produtos."),
        );
    ...
    async (request, reply) => {
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Nome e preco unitario valido sao obrigatorios.",
            ),
          );
      }
      ...
      if (!createResult)
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao criar produto."),
          );
      if (createResult.result === "conflict")
        return reply
          .status(409)
          .send(
            adminError(ADMIN_ERROR_CODE.CONFLICT, "Conflito ao criar produto."),
          );
  ...
  app.put<{ Params: HotelIdParams; Body: ProductUpdateBody }>(
    "/admin/products/:id",
    async (request, reply) => {
      const auth = ensureAuthorizedWithScope(
        request,
      ...
      if (!id)
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id do produto e obrigatorio para atualizacao.",
            ),
          );
      ...
      if (request.body?.category !== undefined)
        payload.category = normalizeOptionalText(request.body.category);
      if (request.body?.status !== undefined)
        payload.status = request.body.status;
      if (request.body?.unit_price !== undefined)
        payload.unit_price = Number(request.body.unit_price);
      ...
      const updateResult = await repository
        .updateProduct(id, activeHotelId, payload)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
        ...
        return reply
          ...
          .send(
            adminError(
              ...
              "Conflito de dados ao atualizar produto.",
            ),
          );
    ...
    async (request, reply) => {
      ...
      if (!id)
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.VALIDATION,
              "Id do produto e obrigatorio para exclusao.",
            ),
          );

      const result = await repository
        .deleteProduct(id, activeHotelId)
        .catch((error) => {
          request.log.error(error);
          return null;
        });
      ...
      if (!result)
        return reply
          .status(500)
          .send(
            adminError(ADMIN_ERROR_CODE.INTERNAL, "Falha ao excluir produto."),
          );
      if (result === "not-found")
        return reply
          .status(404)
      ...
      if (result === "conflict")
        return reply
          ...
          .send(
            adminError(
              ADMIN_ERROR_CODE.CONFLICT,
              "Produto nao pode ser excluido: possui dependencias ativas.",
            ),
          );
