...
describe.sequential("Supabase local com Fastify real", () => {
  ...
  it("consulta todos os modulos e mantem o isolamento entre os dois hoteis", async () => {
    ...
    expect(roomsB.json().items).toEqual(expect.arrayContaining([expect.objectContaining({ id: "20000000-0000-4000-8000-000000000203", status: "maintenance" })]));
  ...
  it("persiste reserva, pagamento, check-in e checkout no fluxo HTTP real", async () => {
    ...
    const maintenanceReference = await app.inject({ method: "GET", url: "/admin/maintenance/reference-data", headers });
    const checkoutOccurrence = await app.inject({ method: "POST", url: "/admin/maintenance/occurrences", headers, payload: { category_id: maintenanceReference.json().categories[0].id, room_id: ROOM_A, stay_id: stayId, kind: "damage", description: "Dano identificado durante o checkout" } });
    expect(checkoutOccurrence.statusCode).toBe(201);
    const checkoutWithoutAcknowledgement = await app.inject({ method: "POST", url: `/admin/stays/${stayId}/checkout`, headers });
    expect(checkoutWithoutAcknowledgement.statusCode).toBe(409);
    const checkout = await app.inject({ method: "POST", url: `/admin/stays/${stayId}/checkout`, headers, payload: { maintenance_acknowledged_occurrence_ids: [checkoutOccurrence.json().item.id] } });
  ...
  it("executa o ciclo de manutenção com duas ordens, bloqueio, inspeção e apuração", async () => {
    const headers = managerHeaders(managerAToken, HOTEL_A);
    const reference = await app.inject({ method: "GET", url: "/admin/maintenance/reference-data", headers });
    expect(reference.statusCode).toBe(200);
    const categoryId = reference.json().categories[0].id as string;
    const { data: manager } = await supabase.from("users").select("password_hash").eq("id", "80000000-0000-4000-8000-000000000002").single();
    const inspectorId = "81000000-0000-4000-8000-000000000001";
    await supabase.from("users").insert({ id: inspectorId, name: "Inspetor Local", email: "inspetor@hotelaria.local", password_hash: manager!.password_hash, is_active: true });
    await supabase.from("user_roles").insert({ user_id: inspectorId, role_id: "70000000-0000-4000-8000-000000000002", hotel_id: HOTEL_A });
    const inspectorToken = await login("inspetor@hotelaria.local");

    const created = await app.inject({ method: "POST", url: "/admin/maintenance/occurrences", headers, payload: { category_id: categoryId, room_id: ROOM_A, kind: "damage", priority: "critical", description: "Dano sintético para validar o ciclo completo" } });
    expect(created.statusCode).toBe(201);
    const occurrenceId = created.json().item.id as string;
    const crossHotel = await app.inject({ method: "GET", url: `/admin/maintenance/occurrences/${occurrenceId}`, headers: managerHeaders(managerBToken, HOTEL_B) });
    expect(crossHotel.statusCode).toBe(404);

    expect((await app.inject({ method: "POST", url: `/admin/maintenance/occurrences/${occurrenceId}/triage`, headers, payload: { priority: "critical" } })).statusCode).toBe(200);
    expect((await app.inject({ method: "POST", url: `/admin/maintenance/occurrences/${occurrenceId}/liability/suspect`, headers, payload: { party: "guest", notes: "Dano identificado após a hospedagem" } })).statusCode).toBe(200);
    const blockStart = addDays(today, 40); const blockEnd = addDays(today, 42);
    const block = await app.inject({ method: "POST", url: `/admin/maintenance/occurrences/${occurrenceId}/room-blocks`, headers, payload: { start_date: blockStart, end_date: blockEnd, status: "maintenance" } });
    expect(block.statusCode).toBe(201);

    const forgedBooking = await app.inject({ method: "POST", url: "/admin/reservations/calendar/booking", headers, payload: { booking_customer: { mode: "existing", customer_id: CUSTOMER_A }, selected_cells: [{ room_id: ROOM_A, date: blockStart, side: "full" }], reservation_source: "front_desk" } });
    expect(forgedBooking.statusCode).toBe(409);

    for (const title of ["Trocar componente", "Testar funcionamento"]) {
      const order = await app.inject({ method: "POST", url: `/admin/maintenance/occurrences/${occurrenceId}/work-orders`, headers, payload: { title, instructions: `${title} conforme diagnóstico técnico`, assigned_to: "80000000-0000-4000-8000-000000000002" } });
      expect(order.statusCode).toBe(201);
    }
    let detail = (await app.inject({ method: "GET", url: `/admin/maintenance/occurrences/${occurrenceId}`, headers })).json().item;
    expect(detail.work_orders).toHaveLength(2);
    for (const order of detail.work_orders as Array<{ id: string }>) {
      expect((await app.inject({ method: "POST", url: `/admin/maintenance/work-orders/${order.id}/transition`, headers, payload: { action: "start" } })).statusCode).toBe(200);
      expect((await app.inject({ method: "POST", url: `/admin/maintenance/work-orders/${order.id}/transition`, headers, payload: { action: "complete", diagnosis: "Falha confirmada", notes: "Reparo concluído" } })).statusCode).toBe(200);
      expect((await app.inject({ method: "POST", url: `/admin/maintenance/work-orders/${order.id}/inspect`, headers, payload: { result: "approved", notes: "Autoinspeção" } })).statusCode).toBe(409);
      expect((await app.inject({ method: "POST", url: `/admin/maintenance/work-orders/${order.id}/inspect`, headers: managerHeaders(inspectorToken, HOTEL_A), payload: { result: "approved", notes: "Serviço aprovado por outro usuário" } })).statusCode).toBe(200);
    }
    detail = (await app.inject({ method: "GET", url: `/admin/maintenance/occurrences/${occurrenceId}`, headers })).json().item;
    const activeBlockId = detail.room_blocks.find((item: { released_at: string | null }) => !item.released_at).id as string;
    expect((await app.inject({ method: "POST", url: `/admin/maintenance/room-blocks/${activeBlockId}/release`, headers, payload: { reason: "Quarto inspecionado e liberado" } })).statusCode).toBe(200);
    expect((await app.inject({ method: "POST", url: `/admin/maintenance/occurrences/${occurrenceId}/liability/decide`, headers, payload: { decision: "confirmed", party: "guest", notes: "Responsabilidade confirmada sem gerar cobrança" } })).statusCode).toBe(200);
    detail = (await app.inject({ method: "GET", url: `/admin/maintenance/occurrences/${occurrenceId}`, headers })).json().item;
    expect(detail.status).toBe("resolved");
    const { count: financialCount } = await supabase.from("financial_transactions").select("id", { count: "exact", head: true }).eq("description", "Responsabilidade confirmada sem gerar cobrança");
