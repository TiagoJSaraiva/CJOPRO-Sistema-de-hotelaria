...
function createSeasonRoomRatesRepositoryMock(overrides: Partial<SeasonRoomRatesRepository> = {}): SeasonRoomRatesRepository {
  return {
    listSeasonRoomRates: vi.fn(async () => []),
    createSeasonRoomRate: vi.fn(async () => ({ result: "ok", item: undefined })),
    updateSeasonRoomRate: vi.fn(async () => ({ result: "ok", item: undefined })),
    deleteSeasonRoomRate: vi.fn(async () => "ok"),
    ...overrides
  };
}

function createSeasonsRepositoryMock(overrides: Partial<SeasonsRepository> = {}): SeasonsRepository {
  return {
...
async function createTestApp(roomRatesRepository: SeasonRoomRatesRepository, seasonsRepository: SeasonsRepository) {
...
describe("routes/season-room-rates with injected repositories", () => {
  it("retorna 404 quando a temporada selecionada nao pertence ao hotel ativo", async () => {
    ...
    expect(seasonsRepository.findSeasonById).toHaveBeenCalledWith("hotel-1", "season-999");
  ...
  it("cria tarifa usando o season_id selecionado quando a temporada existe no hotel", async () => {
    ...
    expect(seasonsRepository.findSeasonById).toHaveBeenCalledWith("hotel-1", "season-1");
    expect(roomRatesRepository.createSeasonRoomRate).toHaveBeenCalledWith("hotel-1", {
      season_id: "season-1",
      room_type: "suite",
      daily_rate: 250
    });
