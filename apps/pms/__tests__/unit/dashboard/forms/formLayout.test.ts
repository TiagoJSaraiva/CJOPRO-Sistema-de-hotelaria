...
describe("dashboard form layout", () => {
  it("usa grid padrao quando formClassName nao e informado", () => {
    expect(getDashboardCreateFormClassName()).toBe(
      DASHBOARD_CREATE_FORM_GRID_CLASS,
    );
  });
  ...
  it("aplica col-span em campos full-width", () => {
    expect(getDashboardFormFieldClassName({ fullWidth: true })).toBe(
      "pms-field md:col-span-2",
    );
  });

  it("concatena classes customizadas mantendo padrao", () => {
    expect(
      getDashboardFormFieldClassName({ fullWidth: true, className: "text-sm" }),
    ).toBe("pms-field md:col-span-2 text-sm");
  });
});
