...
describe("dashboard/users/actions", () => {
  ...
  it("redireciona com forbidden quando usuario nao tem permissao de criacao", async () => {
    ...
    await expect(createUserAction(formData)).rejects.toThrow(
      redirectPattern("/dashboard/users/view\\?status=forbidden"),
    );
  });
  ...
  it("cria usuario e redireciona com status created", async () => {
    ...
    await expect(createUserAction(formData)).rejects.toThrow(
      redirectPattern("/dashboard/users/create\\?status=created"),
    );
  ...
  it("atualiza usuario e redireciona com status updated", async () => {
    ...
    await expect(updateUserAction(formData)).rejects.toThrow(
      redirectPattern("/dashboard/users/view\\?status=updated"),
    );
  ...
  it("remove usuario e redireciona com status deleted", async () => {
    ...
    await expect(deleteUserAction(formData)).rejects.toThrow(
      redirectPattern("/dashboard/users/view\\?status=deleted"),
    );
  ...
  it("redireciona com delete_conflict quando API sinaliza dependencias ativas", async () => {
    ...
    deleteUserMock.mockRejectedValueOnce(
      new Error("Usuario nao pode ser excluido: possui dependencias ativas."),
    );
    ...
    await expect(deleteUserAction(formData)).rejects.toThrow(
      redirectPattern("/dashboard/users/view\\?status=delete_conflict"),
    );
  });
});
