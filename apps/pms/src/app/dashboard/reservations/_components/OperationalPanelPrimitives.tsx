...
export function StatusPill({
  ...
  return (
    <span
      className={`rounded-full border px-[0.55rem] py-[0.2rem] text-[0.76rem] font-semibold ${meta.toneClassName}`}
    >
      {meta.label}
    </span>
  );
...
export function PanelSection({
  ...
  return (
    <section className="grid gap-3 rounded-lg border border-[#e4e7ec] bg-white p-3">
      <div>
        <h4 className="m-0 text-[0.95rem] font-semibold text-[#121926]">
          {title}
        </h4>
        {description ? (
          <p className="mb-0 mt-[0.25rem] text-[0.82rem] text-[#52606d]">
            {description}
          </p>
        ) : null}
      </div>
...
export function PaymentSummaryCard({
  ...
  return (
    <article className={`rounded-lg border p-3 ${toneClassName}`}>
      ...
      <strong className="mt-1 block truncate text-[0.98rem] leading-tight">
        {value}
      </strong>
      <span className="mt-1 block truncate text-[0.76rem] text-[#52606d]">
        {detail}
      </span>
    </article>
  );
