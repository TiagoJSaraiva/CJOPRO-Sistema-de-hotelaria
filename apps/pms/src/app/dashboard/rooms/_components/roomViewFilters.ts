...
export function applyRoomViewFilters(
  rooms: AdminRoom[],
  ...
  return rooms.filter((room) => {
    if (search) {
      const haystack =
        `${room.room_number} ${room.room_type} ${room.notes || ""}`.toLocaleLowerCase();
    ...
    if (
      filters.minOccupancy.trim() &&
      (!Number.isFinite(minOccupancy) || room.max_occupancy < minOccupancy)
    ) {
      return false;
    }

    if (
      filters.maxOccupancy.trim() &&
      (!Number.isFinite(maxOccupancy) || room.max_occupancy > maxOccupancy)
    ) {
      return false;
    }
