...
} from "./DashboardEntityTabsLayout";
...
export function PermissionTabs({
  ...
  return (
    <nav
      ...
      className={`flex flex-wrap gap-[0.55rem]${className ? ` ${className}` : ""}`}
    >
      {items
          ...
          return (
            <Link
              ...
              className={`rounded-full border px-[0.85rem] py-[0.45rem] font-semibold leading-none no-underline ${
                isActive
                  ? "border-[#0f766e] bg-[#dff7f4] text-[#1d2939]"
                  : "border-[#d2d6db] bg-white text-[#1d2939]"
              }`}
            >
              {item.label}
            </Link>
