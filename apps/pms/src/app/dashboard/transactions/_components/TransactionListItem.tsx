...
function DetailItem({ label, value }: { label: string; value: string }) {
  return (
    <div className="min-w-0">
      <span className="block text-[0.72rem] font-semibold uppercase tracking-[0.04em] text-[#52606d]">
        {label}
      </span>
      <strong className="mt-[0.2rem] block truncate text-[0.92rem] font-semibold text-[#202939]">
        {value}
      </strong>
    </div>
  );
...
function TransactionDataPreview({
  transaction,
}: {
  transaction: AdminFinancialTransaction;
}) {
...
function TransactionEditForm({
  ...
  return (
    <form
      ...
      className="mt-4 grid gap-3 border-t border-[#e5e7eb] pt-4"
    >
      ...
      <div className="grid grid-cols-1 gap-3 lg:grid-cols-4">
        ...
        <label className="pms-field">
          ...
          <input
            name="paid_at"
            type="datetime-local"
            defaultValue={toDateTimeLocalValue(transaction.paid_at)}
            className="pms-field-input"
          />
        </label>
        ...
        <label className="pms-field">
          ...
          <input
            name="cost_center"
            defaultValue={transaction.cost_center || ""}
            className="pms-field-input"
          />
        </label>
        ...
        <label className="pms-field">
          ...
          <input
            name="reference_code"
            defaultValue={transaction.reference_code || ""}
            className="pms-field-input"
          />
        </label>
        ...
        <label className="pms-field lg:col-span-2">
          ...
          <input
            name="reservation_id"
            defaultValue={transaction.reservation_id || ""}
            className="pms-field-input"
          />
        </label>
        ...
        <label className="pms-field lg:col-span-2">
          ...
          <input
            name="stay_id"
            defaultValue={transaction.stay_id || ""}
            className="pms-field-input"
          />
        </label>
        ...
        <label className="pms-field lg:col-span-4">
          ...
          <textarea
            ...
            rows={3}
            className="pms-field-input resize-y"
          />
        </label>
      </div>
      ...
      <button
        type="submit"
        className="justify-self-start rounded-lg border border-[#14564c] bg-[#1b7a6c] px-[0.85rem] py-[0.6rem] font-semibold text-white"
      >
        Salvar lançamento
      </button>
    </form>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_150px_150px_170px] xl:items-center">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`rounded-full border px-[0.55rem] py-[0.2rem] text-[0.76rem] font-semibold ${typeClassNames[transaction.type]}`}
            >
              {translateTransactionType(transaction.type)}
            </span>
            <span
              className={`rounded-full border px-[0.55rem] py-[0.2rem] text-[0.76rem] font-semibold ${statusClassNames[transaction.status]}`}
            >
          ...
          <h3 className="mb-[0.2rem] mt-3 truncate text-[1rem] font-semibold text-[#121926]">
            {transaction.category}
          </h3>
          <p className="m-0 truncate text-[0.9rem] text-[#52606d]">
            {transaction.counterparty || "Sem fornecedor"}{" "}
        ...
        <div>
          ...
          <strong
            className={`mt-[0.2rem] block text-[1rem] ${transaction.type === "INCOME" ? "text-[#157347]" : "text-[#b42318]"}`}
          >
        ...
        <div>
          ...
          <strong className="mt-[0.2rem] block text-[0.95rem] text-[#202939]">
        ...
        <div className="flex flex-wrap items-center gap-2 xl:justify-end">
          {canRead ? (
            <Link
              href={viewHref}
              scroll={false}
              className={`rounded-lg border px-[0.65rem] py-[0.45rem] no-underline ${isViewing ? "border-[#2d6cdf] bg-[#e9f0ff] text-[#1b4db3]" : "border-[#d2d6db] bg-white text-[#344054]"}`}
            >
              Ver
            </Link>
          ) : null}
          ...
          {canUpdate ? (
            <Link
              href={editHref}
              scroll={false}
              className={`rounded-lg border px-[0.65rem] py-[0.45rem] no-underline ${isEditing ? "border-[#0f766e] bg-[#ddf5f2] text-[#0a5f58]" : "border-[#d2d6db] bg-white text-[#344054]"}`}
            >
              Editar
            </Link>
          ) : null}
          ...
          {canDelete ? (
            <form action={deleteTransactionAction}>
              ...
              <button
                type="submit"
                className="rounded-lg border border-[#f2b8b5] bg-white px-[0.65rem] py-[0.45rem] text-[#b42318]"
              >
                Excluir
              </button>
            </form>
