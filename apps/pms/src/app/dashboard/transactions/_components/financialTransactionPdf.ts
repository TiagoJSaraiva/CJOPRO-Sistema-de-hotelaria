...
export async function generateFinancialTransactionsPdf(
  input: FinancialTransactionReportInput,
): Promise<FinancialTransactionReportData> {
  ...
  doc.text(
    `Escopo: ${report.scopeLabel} (${report.transactionCountLabel})`,
    12,
    41,
  );
  ...
  if (report.categoryRows.length) {
    ...
    const categoryOptions: UserOptions = {
      ...
      body: report.categoryRows.map((item) => [
        item.category,
        item.amount,
        item.count,
        item.share,
  ...
  const lastAutoTable = (doc as { lastAutoTable?: { finalY?: number } })
    .lastAutoTable;
  const transactionsStartY = Math.max(
    (lastAutoTable?.finalY || tableStartY) + 10,
    tableStartY + 10,
  );
    ...
    body: report.tableRows.length
      ...
      : [
          [
            ...
            "-",
            "-",
            "Sem lançamentos para este escopo.",
            "-",
            "-",
            "-",
            "-",
