...
export function formatUserRoleAssignmentLabel(
  ...
  const hotelLabel = assignment.role_hotel_id
    ? assignment.role_hotel_name || assignment.hotel_name || "Hotel"
    : assignment.hotel_name || assignment.role_hotel_name || "Hotel";
  const genericSuffix = assignment.role_hotel_id ? "" : " (Genérico)";

  return `${hotelLabel} - ${assignment.role_name}${genericSuffix}`;
}

export function formatRoleOptionLabel(
